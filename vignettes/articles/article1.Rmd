...
ggplot() + 
  geom_sf(data=uk_election, aes(fill=country), colour="white") + 
  # geom_sf_label(data=uk_election |> group_by(country) |> summarise(), aes(label=country),size=10) +
  geom_sf(data=uk_election |> filter(constituency_name == "Na h-Eileanan An Iar"), colour="#B20016", fill=NA, linewidth=0.7) + 
  geom_sf_label(data=uk_election |> filter(constituency_name == "Na h-Eileanan An Iar"),
                aes(label=constituency_name)) +
  geom_sf(data=uk_election |> filter(constituency_name == "Orkney and Shetland"), colour="#B20016", fill=NA, linewidth=0.7) + 
  geom_sf_label(data=uk_election |> filter(constituency_name == "Orkney and Shetland"),
                aes(label=constituency_name), nudge_x = -80000, nudge_y = -60000) +
    geom_sf(data=uk_election |> filter(constituency_name=="Ynys Mon"),colour="#B20016", fill=NA, linewidth=0.7) + 
    geom_sf_label(data=uk_election |> filter(constituency_name=="Ynys Mon"),
                  aes(label=constituency_name), nudge_x = -90000) +
    geom_sf(data=uk_election |> filter(constituency_name=="Isle Of Wight"),colour="#B20016", fill=NA, linewidth=0.7) + 
    geom_sf_label(data=uk_election |> filter(constituency_name=="Isle Of Wight"),
...
brms_plot <- ggplot(prep_data2)+geom_sf(aes(fill=brmsfit), linewidth=0.1) +
  scale_fill_gradient2(low="firebrick4",mid="white",high="darkblue",midpoint = 0) +
  coord_sf(datum=NA) + 
  theme_minimal() +
  theme_bw() + 
...
ggsave("/Users/kevinhoran/Documents/sfislands/vignettes/articles/brms_plot.png", width = 12, height = 10)
